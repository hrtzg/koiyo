...
function defineComponent(options, extraOptions) {
	return isFunction(options)
		? // #8236: extend call and options.name access are considered side-effects
			...
			(() =>
				extend({ name: options.name }, extraOptions, {
					setup: options,
				}))()
		: options;
}
	...
	const canSetSetupRef =
		setupState === EMPTY_OBJ
			...
			: key => {
					if (true) {
						if (
							hasOwn(rawSetupState, key) &&
							!isRef2(rawSetupState[key])
						) {
							warn$1(
		...
		for (let i = 0; i < l; i++) {
			...
			if (node) {
				if (isText && !optimized) {
					if (
						i + 1 < l &&
						normalizeVNode(children[i + 1]).type === Text
					) {
						insert(
		...
		__asyncHydrate(el, instance, hydrate2) {
			...
			const doHydrate = hydrateStrategy
				? () => {
						...
						if (teardown) {
							(instance.bum || (instance.bum = [])).push(
								teardown
							);
						}
					}
			...
			if (pendingCacheKey != null) {
				if (isSuspense(instance.subTree.type)) {
					queuePostRenderEffect(() => {
						cache.set(
							pendingCacheKey,
							getInnerChild(instance.subTree)
						);
	...
	if (pendingBranch) {
		...
		if (isSameVNodeType(pendingBranch, newBranch)) {
			...
			if (isInFallback) {
				...
				activeBranch &&
				isSameVNodeType(activeBranch, newBranch)
			) {
				patch(
					activeBranch,
					newBranch,
...
function patchAttr(
	...
	isSVG,
	instance,
	isBoolean2 = isSpecialBooleanAttr(key)
) {
	...
	_mount(def2) {
		...
		for (const key in exposed) {
			if (!hasOwn(this, key)) {
				...
				warn2(
					`Exposed property "${key}" already exists on custom element.`
				);
			}
		}
	}
