...
function computedEager(fn, options) {
	...
	watchEffect(
		...
		},
		{
			...options,
			flush:
				(_a = options == null ? void 0 : options.flush) != null
					? _a
					: 'sync',
		}
	);
...
function formatDate(date, formatStr, options = {}) {
	...
	const meridiem =
		(_a = options.customMeridiem) != null ? _a : defaultMeridiem;
...
function useToggle(initialValue = false, options = {}) {
	...
	function toggle(value) {
		if (arguments.length) {
			...
			_value.value =
				_value.value === truthy ? toValue(falsyValue) : truthy;
			return _value.value;
		}
	}
	...
	const reuse = defineComponent({
		...
		setup(props, { attrs, slots }) {
			return () => {
				...
				return inheritAttrs &&
					(vnode == null ? void 0 : vnode.length) === 1
					? vnode[0]
					: vnode;
			};
		},
	...
	const shouldIgnore = event => {
		return toValue(ignore).some(target2 => {
			if (typeof target2 === 'string') {
				return Array.from(
					window2.document.querySelectorAll(target2)
				).some(
					el =>
	...
	function checkMultipleRoots(target2, event) {
		...
		return children.some(
			child =>
				...
				event.composedPath().includes(child.el)
		);
	}
	...
	const getDeepActiveElement = () => {
		...
		if (deep) {
			while (element == null ? void 0 : element.shadowRoot)
				element =
					(_a2 = element == null ? void 0 : element.shadowRoot) ==
					null
						? void 0
						: _a2.activeElement;
		}
	...
	tasks.reduce((prev, curr) => {
		return prev
			.then(prevRes => {
				...
				if (
					((_a = result[activeIndex.value]) == null
						? void 0
						: _a.state) === promiseState.rejected &&
					interrupt
				) {
					onFinished();
					return;
		...
		promise.value = new Promise((resolve, reject) => {
			try {
				...
				if (_target == null) {
					...
					resolve(
						blobToBase64(
							new Blob([_target], { type: 'text/plain' })
						)
					);
					...
					resolve(
						window.btoa(
							String.fromCharCode(...new Uint8Array(_target))
						)
					);
...
function useBattery(options = {}) {
	...
	if (isSupported.value) {
		navigator2.getBattery().then(_battery => {
			...
			useEventListener(battery, events2, updateBatteryInfo, {
				passive: true,
			});
		});
	}
...
function useMediaQuery(query, options = {}) {
	const { window: window2 = defaultWindow, ssrWidth = useSSRWidth() } =
		options;
	const isSupported = useSupported(
		() =>
			window2 &&
...
function useBreakpoints(breakpoints, options = {}) {
	...
	function current() {
		const points = Object.keys(breakpoints)
			...
			.sort((a, b) => a[2] - b[2]);
		return computed(() =>
			points.filter(([, v]) => v.value).map(([k]) => k)
		);
	}
	return Object.assign(shortcutMethods, {
		...
		greater(k) {
			return useMediaQuery(
				() => `(min-width: ${getValue2(k, 0.1)})`,
				options
			);
		},
		smaller(k) {
			return useMediaQuery(
				() => `(max-width: ${getValue2(k, -0.1)})`,
				options
			);
		},
		between(a, b) {
		...
		isInBetween(a, b) {
			return (
				match('min', getValue2(a)) && match('max', getValue2(b, -0.1))
			);
		},
		current,
		active() {
...
function usePermission(permissionDesc, options = {}) {
	...
	const update = () => {
		var _a, _b;
		state.value =
			(_b = (_a = permissionStatus.value) == null ? void 0 : _a.state) !=
			null
				? _b
				: 'prompt';
	};
		...
		if (!permissionStatus.value) {
			try {
				permissionStatus.value =
					await navigator2.permissions.query(desc);
			} catch (e) {
...
function useCountdown(initialCountdown, options) {
	...
	const reset = countdown => {
		var _a2;
		remaining.value =
			(_a2 = toValue(countdown)) != null
				? _a2
				: toValue(initialCountdown);
	};
	const stop = () => {
		intervalController.pause();
		reset();
...
function useCssVar(prop, target, options = {}) {
	...
	function updateCssVar() {
		...
		if (el && window2 && key) {
			const value =
				(_a = window2.getComputedStyle(el).getPropertyValue(key)) ==
				null
					? void 0
					: _a.trim();
...
function useCycleList(list, options) {
	...
	const index = computed({
		get() {
			...
			if (index2 < 0)
				index2 =
					(_a = options == null ? void 0 : options.fallbackIndex) !=
					null
						? _a
						: 0;
			return index2;
	...
	function init() {
		if (window2) {
			const onDeviceMotion = createFilterWrapper(eventFilter, event => {
				...
				acceleration.value = {
					x:
						((_a = event.acceleration) == null ? void 0 : _a.x) ||
						null,
					y:
						((_b = event.acceleration) == null ? void 0 : _b.y) ||
						null,
					z:
						((_c = event.acceleration) == null ? void 0 : _c.z) ||
						null,
				};
				...
				rotationRate.value = {
					alpha:
						((_g = event.rotationRate) == null
							? void 0
							: _g.alpha) || null,
					beta:
						((_h = event.rotationRate) == null
							? void 0
							: _h.beta) || null,
					gamma:
						((_i = event.rotationRate) == null
							? void 0
							: _i.gamma) || null,
				};
...
function useDevicesList(options = {}) {
	...
	async function ensurePermissions() {
		...
		if (state.value !== 'granted') {
			...
			try {
				stream =
					await navigator2.mediaDevices.getUserMedia(constraints);
			} catch (e) {
				stream = null;
				granted = false;
			}
...
function useDisplayMedia(options = {}) {
	...
	const isSupported = useSupported(() => {
		var _a2;
		return (_a2 = navigator2 == null ? void 0 : navigator2.mediaDevices) ==
			null
			? void 0
			: _a2.getDisplayMedia;
	});
	...
	async function _start() {
		...
		stream.value =
			await navigator2.mediaDevices.getDisplayMedia(constraint);
	...
	const move = e => {
		...
		if (axis === 'x' || axis === 'both') {
			...
			if (container)
				x = Math.min(
					Math.max(0, x),
					container.scrollWidth - targetRect.width
				);
		}
		if (axis === 'y' || axis === 'both') {
	...
	if (isClient) {
		...
		const getFiles = event => {
			...
			const list = Array.from(
				(_b2 =
					(_a2 = event.dataTransfer) == null ? void 0 : _a2.files) !=
					null
					? _b2
					: []
			);
		...
		const handleDragEvent = (event, eventType) => {
			...
			isValid =
				(_b2 =
					...
					checkValidity(dataTransferItemList)) != null
					? _b2
					: false;
...
function useElementSize(
	...
	const isSVG = computed(() => {
		var _a, _b;
		return (_b =
			(_a = unrefElement(target)) == null ? void 0 : _a.namespaceURI) ==
			null
			? void 0
			: _b.includes('svg');
	});
	...
	tryOnMounted(() => {
		...
		if (ele) {
			width.value =
				'offsetWidth' in ele ? ele.offsetWidth : initialSize.width;
			height.value =
...
function useFavicon(newIcon = null, options = {}) {
	...
	const applyIcon = icon => {
		...
		if (!elements || elements.length === 0) {
			const link =
				document2 == null ? void 0 : document2.createElement('link');
			if (link) {
				link.rel = rel;
...
function useFileSystemAccess(options = {}) {
	...
	const fileSize = computed(() => {
		var _a, _b;
		return (_b = (_a = file.value) == null ? void 0 : _a.size) != null
			? _b
			: 0;
	});
	const fileLastModified = computed(() => {
		var _a, _b;
		return (_b = (_a = file.value) == null ? void 0 : _a.lastModified) !=
			null
			? _b
			: 0;
	});
...
function useFullscreen(target, options = {}) {
	...
	const isCurrentElementFullScreen = () => {
		if (fullscreenElementMethod)
			return (
				(document2 == null
					? void 0
					: document2[fullscreenElementMethod]) === targetRef.value
			);
		return false;
	};
		...
		if (fullscreenEnabled.value) {
			if (document2 && document2[fullscreenEnabled.value] != null) {
				...
				if (
					(target2 == null
						? void 0
						: target2[fullscreenEnabled.value]) != null
				) {
	...
	async function exit() {
		...
		if (exitMethod.value) {
			if (
				(document2 == null ? void 0 : document2[exitMethod.value]) !=
				null
			) {
				...
				const target2 = targetRef.value;
				if (
					(target2 == null ? void 0 : target2[exitMethod.value]) !=
					null
				)
...
function useScroll(element, options = {}) {
	...
	function scrollTo(_x, _y) {
		...
		if (!_element) return;
		(_c =
			_element instanceof Document ? window2.document.body : _element) ==
		null
			? void 0
			: _c.scrollTo({
	...
	const onScrollHandler = e => {
		...
		if (!window2) return;
		const eventTarget =
			(_a = e.target.documentElement) != null ? _a : e.target;
...
function useClamp(value, min, max) {
	if (typeof value === 'function' || isReadonly(value))
		return computed(() =>
			clamp(toValue(value), toValue(min), toValue(max))
		);
	const _value = ref(value);
	return computed({
		get() {
			return (_value.value = clamp(
				_value.value,
				toValue(min),
				toValue(max)
			));
...
function useScreenOrientation(options = {}) {
	...
	const screenOrientation = isSupported.value
		? window2.screen.orientation
		: {};
...
function useParallax(target, options = {}) {
	...
	const screenOrientation = reactive(
		useScreenOrientation({ window: window2 })
	);
	const {
		elementX: x,
		elementY: y,
...
function usePointerLock(target, options = {}) {
		...
		useEventListener(
			...
			() => {
				...
				const currentElement =
					(_a = document2.pointerLockElement) != null
						? _a
						: element.value;
			...
			() => {
				...
				const currentElement =
					(_a = document2.pointerLockElement) != null
						...
						: element.value;
				if (targetElement && currentElement === targetElement) {
					const action = document2.pointerLockElement
						? 'release'
						: 'acquire';
	...
	async function lock(e) {
		...
		if (!isSupported.value)
			throw new Error(
				'Pointer Lock API is not supported by your browser.'
			);
...
function useSpeechRecognition(options = {}) {
	...
	const SpeechRecognition =
		window2 &&
		(window2.SpeechRecognition || window2.webkitSpeechRecognition);
				...
				try {
					if (data.value == null) await storage.removeItem(key);
						...
						await storage.setItem(
							key,
							await serializer.write(data.value)
						);
				} catch (e) {
					onError(e);
				}
...
function useStyleTag(css, options = {}) {
	...
	const load = () => {
		if (!document2) return;
		const el =
			document2.getElementById(id) || document2.createElement('style');
		if (!el.isConnected) {
			el.id = id;
	...
	function getValue2() {
		...
		return (_b =
			(_a =
				document2 == null
					? void 0
					: document2.querySelector(selector)) == null
				? void 0
...
function useTextareaAutosize(options = {}) {
	...
	const styleProp =
		(_b = options == null ? void 0 : options.styleProp) != null
			? _b
			: 'height';
...
function useTitle(newTitle = null, options = {}) {
	...
	tryOnScopeDispose(() => {
		if (restoreOnUnmount) {
			const restoredTitle = restoreOnUnmount(
				originalTitle,
				title.value || ''
			);
...
function executeTransition(source, from, to, options = {}) {
	...
	return new Promise(resolve => {
		...
		const tick = () => {
			...
			const arr = toVec(source.value).map((n, i) =>
				lerp(v1[i], v2[i], alpha)
			);
...
function useUserMedia(options = {}) {
	var _a, _b;
	const enabled = shallowRef((_a = options.enabled) != null ? _a : false);
	const autoSwitch = shallowRef(
		(_b = options.autoSwitch) != null ? _b : true
	);
...
function useVModel(props, key, emit, options = {}) {
	...
	const _emit =
		...
		((_a = vm == null ? void 0 : vm.$emit) == null
			? void 0
			: _a.bind(vm)) ||
		((_c =
			(_b = vm == null ? void 0 : vm.proxy) == null
				? void 0
				: _b.$emit) == null
			? void 0
	...
	if (isSupported.value) {
		useEventListener(
			...
			() => {
				var _a, _b;
				requestedType.value =
					(_b = (_a = sentinel.value) == null ? void 0 : _a.type) !=
					null
						? _b
						: false;
			},
...
function useWebWorkerFn(fn, options = {}) {
	...
	const callWorker = (...fnArgs) =>
		new Promise((resolve, reject) => {
			...
			(_a = worker.value) == null
				? void 0
				: _a.postMessage([[...fnArgs]]);
